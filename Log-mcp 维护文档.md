...
**源码：****http://172\.16\.91\.163/dksz/log\-mcp**
...
**时间参数统一**：所有日志查询工具均支持 `startDate` \+ `endDate`（日期范围）与 `timeRange`（最近 N 分钟/小时）两种互斥方式，都不填则默认查询最近 **30 分钟**。返回结果头部会展示实际查询的时间段。



**索引优化**：只要指定了 `startDat` / `endDate`，统一走通配符索引（如 `springcloud-*-logs-*`）。因为 ELK/Logstash 默认按 **UTC** 滚动索引，一个本地日期可能对应前后两个 UTC 日期；精确到单日容易漏查。实际过滤由 `@timestamp`  range 查询完成，不会返回其他日期的数据。
